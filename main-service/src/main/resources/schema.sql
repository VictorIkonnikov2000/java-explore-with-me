...
CREATE TABLE IF NOT EXISTS comments (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    text VARCHAR(2000) NOT NULL,  -- Текст комментария. Ограничение в 2000 символов, как и в  `events.annotation`.
    event_id BIGINT NOT NULL,       -- ID события, к которому относится комментарий.
    author_id BIGINT NOT NULL,      -- ID пользователя, оставившего комментарий.
    created_on TIMESTAMP WITHOUT TIME ZONE NOT NULL, -- Дата и время создания комментария.
    updated_on TIMESTAMP WITHOUT TIME ZONE,         -- Дата и время последнего обновления комментария (может быть NULL).
