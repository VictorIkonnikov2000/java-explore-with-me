...
public interface EndpointHitRepository extends JpaRepository<EndpointHitEntity, Long> {
    @Query("select new ru.practicum.ewm.stats.dto.ru.practicum.ewm.stats.dto.ViewStatsDto(h.app, h.uri, count(h.id)) "
    ...
    @Query("select new ru.practicum.ewm.stats.dto.ru.practicum.ewm.stats.dto.ViewStatsDto(h.app, h.uri, count(distinct h.ip)) "
            + "from EndpointHitEntity h "
            + "where h.timestamp between :start and :end "
            + "and (:uris is null or h.uri in :uris) "
            + "group by h.app, h.uri "
